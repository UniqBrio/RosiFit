# Canonical Patterns — framework v1 (SUPERSEDED-AT-ADOPTION)

> **SUPERSEDED-AT-ADOPTION — 02-Sep-2026. Do not add rows here. Do not follow these rows.**
>
> This is the `custom-web-app-development-framework` v1.3.0 seed copy of
> `CANONICAL_PATTERNS.md`, exactly as it arrived when RosiFit was bootstrapped from the
> framework. It is retained as history, not as guidance.
>
> **Why it was superseded rather than amended.** RosiFit is an *adopted* app, not a scaffolded
> one: there is no `starter/`, so none of the reference files these twenty-one rows name were
> ever copied in. `check-rule-coverage.mjs --report` scored it **27 rules, 20 dead rungs** — CP-2
> through CP-21 each pointed at a file that does not exist in this repository. Twenty dead rungs
> is not honest debt; it is twenty rules claiming enforcement none of them had, which is the
> exact failure the rule-coverage audit exists to detect.
>
> Amending twenty rows in place would have destroyed the record of what the framework actually
> shipped. Superseding preserves both: the seed is here, RosiFit's own patterns start clean at
> **CP-001** in [../CANONICAL_PATTERNS.md](../CANONICAL_PATTERNS.md).
>
> **This file is not read by any gate.** `scripts/audits/check-rule-coverage.mjs` parses a
> literal three-path register list (`CANONICAL_PATTERNS.md`, `ROOT_CAUSE_REGISTER.md`,
> `DESIGN_RULES.md`) and performs no directory descent, so nothing under `_archive/` is counted.
> The one standing constraint on this directory: **never archive a file whose basename matches a
> rung-claimed source file**, because the audit's rung-existence check falls back to a
> basename-anywhere search that would then report a dead rung as live. Markdown archives are safe.
>
> Superseded by: DECISION_LOG 002. Numbering note: the framework used `CP-N`; RosiFit uses
> `CP-NNN`. The two spaces never collide, so no ID is ever reused.
