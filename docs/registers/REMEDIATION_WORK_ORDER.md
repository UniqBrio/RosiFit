...
**D-9b (18-Sep-2026) — Each session works in its own git worktree or clone.** `git worktree add ../RosiFit-<X> <branch>` — shared object store, separate HEAD and index. No session checks out another's branch. Everyone branches from `origin/main` and pushes `HEAD:<branch>`. Added 18-Sep after `fix/T-014`'s second commit was moved by another session's checkout in a shared worktree: the branch ref was silently rewound to a rebased copy of its first commit and the spec file was left untracked on disk. `origin` had the real history, so nothing was lost — but nothing in the tree said so either.
**D-2b (18-Sep-2026) — a spec whose premise has been disproved is not a protected lock.** `src/data/aliasConflictTarget.test.ts` resolves function bodies by reading migration FILES and scanning them for `ON CONFLICT` targets. RC-047 established that a file is not what the database runs: 0073 became an in-place edit precisely because restating a body from a file reverts every edit made since. The scan is therefore asking the wrong source, and has failed ×5 since 0073 changed shape. `51_alias_unique_per_member.sql` already asserts the same class IN THE DATABASE, after a full replay, which is the only place the question can be answered. The source scan is retired, not repaired. The append-only rule protects specs that still assert something true; it is not a reason to keep a check whose premise the register has disproved. (T-121, T-111, RC-047.)
**D-10 (18-Sep-2026) — a migration merges to `main` only on the day it is applied to production, with its ledger row.** Files merged but unapplied are how twelve migrations became replay fiction: the harness replays them, production has never seen them, and every green rehearsal since has been proving a schema nobody runs (T-120). A migration file on `main` is a claim that the database is in that state. Until the apply, the file lives on its branch.
**D-11 (18-Sep-2026) — the harness replays the REPOSITORY, not production's applied set.** T-120 measured the gap: 60 application functions live, 62 in the replay, 43 identical, 15 different, 2 present only in the harness. Until those differences are reconciled, a green result about any of the 15 is not evidence about production — it is evidence about a database nobody runs. A second replay mode applies only ledger-confirmed migrations; the twelve unapplied files are a named pending set, listed in T-120's row and worked one row at a time.
**D-12 (18-Sep-2026) — sessions merge their own PRs.** Conditions, all three: `db-harness` green; `gate` red **only** on the nine RV-03 failures; and the RC entry present in `ROOT_CAUSE_REGISTER.md`. Production applies still require the owner's explicit go-ahead, unchanged. Note on the second condition as it stands today: `npm run check` fails **14**, not nine — five of them are `aliasConflictTarget.test.ts`, retired by T-121 under D-2b. Until that lands, no PR meets the gate condition, so T-121 is on the critical path for every other session's merges.
**D-12a (18-Sep-2026) — merge conditions are relative to `main`'s baseline, not to absolute green.** `db-harness`: the failure set is identical to or smaller than `main`'s, AND the PR's own specs are green. `gate`: the failure set is identical to or smaller than `main`'s **in CI** (T-302: CI sees 8 after T-121, not 9), AND nothing the PR adds fails. RC entry present. Absolute green stays where D-7 put it, at Gate 2 exit. Rationale: without this, the first row that would fix a standing failure can never merge the fix that makes it mergeable.
**D-12b (18-Sep-2026) — a production apply requires BOTH jobs of the PR's latest run to have been read first.** `gate` and `db-harness`, including the gate's failure diff against `main`, both pasted, before `supabase db query` runs. The apply is not authorised until they are. Written after RC-052: `migrationGrants.test.ts:102` had already failed on the PR's own run, naming the function and quoting the correct statement, two hours before 0075 was applied with `anon` able to execute it. The rung worked; the person read one job of two. A migration's grants are checked only in the `gate` job, because the spec that checks them lives under `src/`.
**D-12 addendum (Session B, 18-Sep-2026) — measured, and it blocks every session.** Two of the three conditions cannot be read as written:

1. **`db-harness` green is not satisfiable by any PR today.** The latest run on `main` itself (`35332129859`, commit `c1a9a74`) is `db-harness: failure`. Session B's `fix/T-018` run (`35332977464`) produces the **byte-identical** failure set — 10 distinct `FAIL` assertions, diffed and confirmed identical — and that branch touches no SQL, no migration and nothing under `db/harness`. Every commit landed on `main` since these branches was a register edit. So the condition is not failing because of any PR; the SQL suite has been red since CI first executed on 11-Sep (T-001, T-012), and **D-7 already assigns "`npm run test:db` fully green" to Gate 2's exit, not to Gate 1.** As written, condition 1 stops all three sessions merging anything. It needs either a ruling that it means *unchanged by this PR* — the same tolerance condition 2 already grants the gate — or Gate 2 first.

2. **The "nine RV-03 failures" is a Windows count. CI sees eight.** `memberJoinedOn.test.ts:141` slices `src.slice(at, at + 600)` out of `app/member/edit.tsx`. That window spans **11 lines on a CRLF checkout and 12 on LF**, and the twelfth line is the one carrying `testID="member-joined-on"`. So it fails locally and passes in CI, for no reason connected to the code it is about. Measured counts, `gate` job: `main` and all four of Session B's branches **13**; `fix/T-121` **8**. `13 = 8 + 5`, the five being `aliasConflictTarget.test.ts`. Both register numbers are one too high because both were taken on Windows. The class is filed as **T-302**.

**What this means for the merge queue:** condition 3 (RC entry) is met on all six of Session B's PRs; condition 2 is met once T-121 is in; condition 1 is met by nobody. No Session B PR has been merged, and none will be until the owner rules on condition 1.

**D-13 (18-Sep-2026) — The primary worktree stays on `main`, and is fast-forwarded before any push from any worktree.** `git pull --ff-only` in the primary tree, first thing, and never leave a feature branch checked out there. Rationale, measured the same day (T-401): the PreToolUse guard resolves the repository from the hook’s own working directory, which is the session’s primary tree — not the worktree the command targets. With the primary tree five commits behind `main` on another session’s branch, `@{u}..HEAD` holds no commits to carry an escape token but a non-empty reversed diff, so every guard sees a change and none can find a justification. The push is then unescapable by anything the pushing session writes. Worse in the other direction: while a stale checkout persists anywhere, the whole guard suite is effectively disabled for every session, and a push that passes proves nothing about any guard. This is the INTERIM rule and it is a workaround, not the fix — T-401 makes the hook read the command’s target worktree, at which point this rule becomes hygiene rather than load-bearing.
**D-14 (18-Sep-2026) — No `supabase functions deploy`, by any session, until every deployed function has been downloaded and diffed against `supabase/functions/`.** T-400 is the gate. No deployed Edge Function has ever been compared with this repository: the eleven were deployed by hand at different times from different working copies, three different upload roots are still visible in the platform’s entrypoint paths, and versions run from v4 to v18 with several predating migrations that changed the tables they read. T-120 found exactly this class one layer down, where 15 database functions in production differ from a full harness replay. Until the diff is taken, “the function does X, because the source says so” is unfounded — including in T-020, T-038 and T-041. **The diff must precede the first deploy, because a deploy destroys the evidence it would be based on.** `supabase/config.toml` (T-038) merges regardless and changes nothing on its own; it takes effect at the next deploy, which is now gated on T-400. T-127’s `verify_jwt` correction is made with the dashboard toggle for the same reason: it changes the gateway setting without pushing repository code over whatever is actually running.
