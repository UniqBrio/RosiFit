...
| TD-010 | **The only automated target cannot be run.** `ENVIRONMENTS.md` names the local harness *"the only automated target"* — production is never one, and there is no staging. But neither `psql` nor Docker is on PATH on the adopting machine, so `bash db/harness/test.sh` aborts in `reset.sh`. **The one environment the process is allowed to write to is currently unreachable.** | An environment gap, not a code one, and pre-existing: it failed identically before the first commit on this branch, so nothing in this pass caused it. Accepted because this pass changed nothing under `supabase/` or `db/`. | The 124 SQL assertions cannot be executed, so **no DB-adjacent change can be proven at all on this machine** — not merely inconvenienced. Every rule in `ENVIRONMENTS.md` that depends on "applied to the harness first and confirmed" is unenforceable until this is fixed, which makes it a governance gap and not just a tooling one. | Postgres 16 (or Docker) is installed locally, **or** the harness runs in CI — the latter is the more durable fix, because it does not depend on any one machine. | 02-Sep-2026 |
