...
| TD-012 | **Five more forms flash a confirmation for a write that does not happen** — `app/holiday.tsx`, `app/member/edit.tsx`, `app/course/rules.tsx`, `app/staff/add.tsx`, `app/templates.tsx`. Same shape as RC-008, which was fixed in `app/course/edit.tsx`. | Deliberate scope: the reported defect was Add Course, and each of these needs its own write path, its own RLS-refusal message and its own list revalidation. Fixing five at once, unasked, would have been a much larger change than the one requested. | **The highest-cost row in this table.** Every one of them is a user believing a record exists that does not, and each will be found the same way RC-008 was — from a user report, one at a time. `app/holiday.tsx` is the worst: it also states a session-impact count that is hardcoded (`14`/`6`) rather than queried. | Each gets a repository write, an awaited save, a rendered refusal and an `onChanged` revalidation, following `app/course/edit.tsx`. Best done as one pass over all five. | 02-Sep-2026 |
