...
|---|---|---|
| email (AWS SES via `send-followups`) | `support@getfit.rosifit.com` **and `support+*@getfit.rosifit.com`** | **Added 07-Sep-2026, and SUPERSEDED the same day** — the owner clarified that `support@` is the SENDER and members keep their own addresses, so `seed_send_test.sql` is not the route taken. The row stands because the destination is still approved and the script still works if a redirect is ever wanted. Originally added in the change that made it reachable. The academy's own mailbox on its own SES-verified domain. Plus-addressing is what lets fifteen member records point at one inbox without colliding on `member_emails_unique_live`. Reached only via `supabase/seed_send_test.sql`; reverse with its teardown. |
| email (AWS SES via `send-followups`) | **`shaziafarheen74@gmail.com`** and **`uniqbotzinfo@gmail.com`** | **Added 07-Sep-2026, on the repo owner's instruction.** These are the ONLY two recipient identities verified in SES (`ap-south-1`), so while the account is in the sandbox they are the only addresses that can receive at all — every other member fails with `Email address is not verified`. Verification required someone to click a confirmation link sent to each, so control of both is demonstrated rather than assumed. Reached via `supabase/seed_followup_test.sql`; reverse with its teardown. |
