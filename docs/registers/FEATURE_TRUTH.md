...
| Mark one member present or absent, on the roster | ◻ | **The first way a person may write attendance.** Three chips on every roster card — **Present · Absent · Yet to mark** — for the day the week strip has selected, named once under the search box (*Attendance for Mon 7 Sept*). One tap, no dialog. Until 07-Sep-2026 attendance had exactly one way in, a Google Meet export committed by `commit_csv_import`, and **no way to correct**: a member who joined from another device was missing from the file, recorded absent, counted towards the follow-up rule and emailed about a class she attended. **What the chips may not do is the design.** *Yet to mark* is a STATE, never a control — clearing a record is a hole in the register, not a correction. *Absent* is refused on a day she was not expected (`absent_must_be_expected`, 0008) and says why in the chip's own label; *Present* on such a day is stored as `extra`, because that is what `extra` means. A future date offers nothing. A cancelled class and a holiday are refused server-side. The chip fills on the WRITE's resolution and never on the tap (RC-008, RC-017 — both recorded in these files). All of it goes through **`set_attendance` (0035)**, `security definer`: it resolves her offering from the enrolment in force, finds-or-creates that day's session with 0024's own `expectation_mode` rule, derives `expected` from `expected_members_for_session()` so the caller cannot assert it, and stamps `original_status`/`corrected_by`/`corrected_at` when it changes what the register already said — four columns 0008 created and nothing had ever written. **No table grant moved:** `authenticated` still holds only SELECT on `attendance_records` (RBAC_MATRIX, and RC-007 is why that matters). 24 assertions in `supabase/tests/27_set_attendance.sql` — **written, not yet run**: this machine has no PostgreSQL 16, so the harness could not rehearse 0035, and **0035 is not applied to production** (TD-033). Until it is, the chips READ correctly and a tap says so in words — *"The academy database cannot record attendance by hand yet — migration 0035 has not been applied"* — rather than reporting a save it did not make. The 13 specs in `src/data/dayAttendance.test.ts` DO run and pass, and they cover every state the chips can be in. `requests/2026-09-07-member-card-attendance-state.md` |
