...
| Edit a member | ◻ | **Her name is the only required field.** **Edit is Edit from the moment it opens — 06-Sep-2026.** The dialog reads the route, not the result of its own lookup, so while her record is still being fetched it says *Edit member · Fetching her record* over a skeleton with no Save, a failed read offers Try again, and an id that is not on the register is told plainly. It used to answer all three with the ADD form — blank, titled *Welcome a new member*, over a Save that would have created her a second time (RC-021, and RC-012 before it) |
