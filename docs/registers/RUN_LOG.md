...
|---|---|---|---|---|---|---|---|---|---|---|
| R-008 | T-049 fail-first spec - three drain cases, A-side spec into T-049, T-128 filed for B | CHANGE | n/a | 2026-09-19 13:16 | 2026-09-19 13:19 | 3m elapsed · active: no marks | - | 27.5s | FAIL | back-filled start; Gate FAIL is pre-existing and unchanged from the previous run: G1 no design/tokens.json, G6 no local eslint runner, G7 an app/(tabs)/courses.tsx spec on B's surface, G8 test:functional. Row back-filled - the run was not opened at the start. |
