...
|---|---|---|---|---|---|---|---|---|---|---|
| R-022 | Fix 4 (T-407): split the JavaScript per screen. Target: under 250 KB compressed per screen, LCP under 2.5 s on the mobile profile for Home, Attendance, Members | CHANGE | scoped | 2026-09-25 11:00 | 2026-09-25 11:38 | 37m elapsed · active: no marks | - | 29.0s | FAIL | T-407: 702->467 KB/screen, LCP Home 3.95 s, Members 5.80 s; owner targets not met; gate FAIL = pre-existing set |
| R-021 | RosiFit — investigate and experimentally optimize Supabase Edge Function region (csv-import to Singapore; measure before/after; no other changes) | CHANGE | scoped | 2026-09-24 11:12 | 2026-09-24 11:12 | 0s active · 0s elapsed | gate 0s | 24.3s | FAIL | T-408 built: csv-import pinned to ap-southeast-1 via forceFunctionRegion; fail-first observed; gate FAIL only on the pre-existing set (G1-3 T-034, G6 conformance.mjs warning, G7 6 main failures, G8); after-numbers need real imports post-deploy |
| R-021 | Fix 3 (T-406): no cross-screen dedupe — ~116 requests per Home load, same data fetched repeatedly; trailing empty page on every paged read; re-measure after | BUG | scoped | 2026-09-25 04:48 | 2026-09-25 05:11 | 10m active · 24m elapsed | gate 0s | 27.3s | FAIL | T-406 built and measured locally: cold start+3 tabs 54->31 requests, cold start 1.83s->0.70s; 5 s window chosen from 2/5/12 s measurement; gate FAIL only on the pre-existing set; production on merge |
| R-021 | Fix 2 (T-405): the signed-in identity is read 10 times one after another before any data (2.4 s); re-measure after | BUG | scoped | 2026-09-25 04:29 | 2026-09-25 04:45 | 10m active · 16m elapsed | gate 0s | 32.8s | FAIL | T-405 built and measured locally: identity reads 10->2, cold start 1.83s->0.71s (200 ms stand-in); gate FAIL only on the pre-existing set; production verification on merge |
| R-016 | The app feels slow even with little data. Measure before fixing, report numbers: region vs India; per-screen call count/waterfall; initial JS + Lighthouse mobile for 3 most-used screens; Supabase performance advisors. Rank causes by measured cost. Fix the top ones only after I see the numbers. | BUG | full-scale | 2026-09-24 04:43 | 2026-09-24 07:05 | 20m active · 2h 22m elapsed | ground 54m · verify 1h 28m · gate 0s | 26.3s | BLOCKED | T-043/0079 built and proven on the harness (spec 58 28/28, fail-first 62->0, equivalence re-proved in-migration); harness red only on the pre-existing Gate 2 set; production apply awaits the owner's go-ahead on the raw SQL |
