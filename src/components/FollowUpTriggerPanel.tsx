...
import {
  ...
  triggerConverts, triggerDirty, triggerResettable, triggerSavedLine, triggerSummary,
  type TriggerReading,
...
export function FollowUpTriggerPanel({
  ...
  /* EXPANDED, EVERY TIME. On request: "by default it should be expanded all
     time" -- so this is plain component state and is deliberately NOT
     remembered anywhere. Every mount of this panel, on every screen and on
     every open of the dialog that holds it, starts open.

     Nothing collapses it but a press. In particular Apply does not: the ask
     was "after they apply they can MANUALLY click on it so that it shrink",
     which is a control offered after the work is done, not a card that folds
     itself away the moment something lands. A panel that moved on its own
     while a person was reading the number it holds would be the opposite of
     what this is for. */
  ...
  return (
    <View testID={testID} style={{
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: SPACE.sm }}>
        ...
        {/* THE CHEVRON, and a word beside it. The direction of the glyph is
            the same one every other disclosure in this app uses (TokenChips,
            the audit groups), and it never carries the state alone --
            guardrail 3 is why "Hide"/"Show" is drawn and not only spoken.
            `expanded` is announced too, so the control says what it is to a
            screen reader before it says what it does. */}
        <Pressable testID={`${testID}-toggle`}
          onPress={() => setCollapsed(c => !c)}
          accessibilityRole="button"
          accessibilityState={{ expanded: !collapsed }}
          accessibilityLabel={collapsed
            ? 'Show the follow-up trigger controls'
            : 'Hide the follow-up trigger controls'}
          style={({ pressed }) => ({
            flexDirection: 'row', alignItems: 'center', gap: 3,
            minHeight: 24, paddingHorizontal: 4, opacity: pressed ? 0.7 : 1,
          })}>
          <Icon name={collapsed ? 'expand_more' : 'expand_less'} size={16} color={theme.accentInk} />
          <Text style={{ fontSize: 11, fontWeight: '800', color: theme.accentInk }}>
            {collapsed ? 'Show' : 'Hide'}
          </Text>
        </Pressable>
      </View>
      ...
      {collapsed ? (
        /* SHRUNK, NOT SILENT. The heading alone would leave a card that says
           there is a rule without saying what it is, over a list the rule
           produced. The number in force stays, generated by the same module
           as every other word here (C-67), and a typed-but-unapplied change
           is NAMED -- with the stepper folded away there is nothing else on
           screen that could say so. */
        <Text testID={`${testID}-summary`} style={{
          fontSize: 12.5, lineHeight: 18, fontWeight: '600',
          color: theme.fgStrong, marginTop: SPACE.sm,
        }}>{triggerSummary(reading, shown)}</Text>
      ) : (
      <>
      ...
      {/* A FAILED SAVE OUTLIVES THE COLLAPSE, which is why it sits out here
          rather than inside the expanded branch with the controls. Shrinking
          the card is a tidying press; it must not be a way to make "nothing
          has been changed" disappear off a screen whose next press sends
          email. `failure` is only ever set by apply(), which needs an
          editable panel, so the read-only card can never draw it. */}
      {failure ? (
        <View style={{ flexDirection: 'row', gap: SPACE.sm, marginTop: SPACE.sm }}>
          <Icon name="error" size={15} color={dangerInk} />
