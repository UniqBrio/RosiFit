// Cold-start waterfall reproduction (T-405, RUN_app-feels-slow.md). Not part of any gate.
...
//      (EXPIRED=1 starts from an expired token; cold-start-nocache.js with NOCACHE=1 disables the browser cache)
//   h2.log: start ms, end ms, HTTP version, method, path -- one line per request.
...
srv.on('request', (q, r) => {
  ...
  setTimeout(() => {
    ...
    fs.appendFileSync(LOG, `${start}\t${Date.now() - t0}\t${q.httpVersion}\t${q.method}\t${path}\n`);
