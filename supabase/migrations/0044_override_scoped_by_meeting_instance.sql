-- 0044 · several files for one course on one day, and none of them erases another
--
-- THE ASK
...
--
-- WHAT ALREADY DID THIS, and is not touched
--   Consolidation is the shape the schema has had since 0007/0008 and nothing
--   here loosens it:
--     * sessions_unique_live (offering_id, session_date) -- N files for a day
--       land on ONE session; a second file cannot create a rival one.
--     * attendance_unique_live (session_id, member_id) where deleted_at is
--       null -- one row per member per day, so "no duplicate attendance
--       records" is a database invariant and not bookkeeping.
--     * the upsert marks this file's people present; the absent sweep is
--       `on conflict do nothing`, so anybody an EARLIER file today marked
--       present stays present. Union, file after file, which is the
--       requester's own flow: "when they upload again, check the members
--       marked absent: if they are in that file, mark them present."
--
-- WHAT DID NOT, and is the whole of this migration
--   0037 made a second file for a day a full override -- everyone due whose
--   row an earlier file wrote and this file does not name goes back to absent,
--   and an 'extra' in the same position is soft-deleted. That is right for a
--   CORRECTED RE-EXPORT and destructive for a SECOND MEETING, so 0042 keyed it
--   on the meeting code: only a file carrying the same code overrides.
--
--   The code is the Meet LINK, not the call. An academy that keeps one link
--   for a course -- the ordinary case for a recurring class -- writes the same
--   code on every export, so two genuine meetings on one day were read as a
--   file and its correction, and the second erased the first. Files whose
--   export carries no code line at all were worse off still: `is not distinct
--   from` makes null match null, so EVERY code-less file overrode every other
--   code-less file for that day.
--
--   Both are the case the ask names, and in both the reverted member really
--   attended: her class ran, her file said so, and a later file about a
--   different call took it away.
--
-- THE DISCRIMINATOR
--   The meeting INSTANCE: the code AND the created-on timestamp Meet writes
--   above the table, recorded in csv_imports.meeting_started_at since 0024.
--   Two calls on one link have two different created-on lines. A re-export of
--   one call has the same one -- it is a property of the meeting, not of the
--   export -- so a corrected file still corrects its own earlier version in
--   full, which is what 0037 was asked for and what 29_import_override.sql
--   holds it to.
--
--   `is not distinct from` on both halves, deliberately, for the reason 0042
--   gives about the code: it keeps the behaviour of every row already in the
--   table, including the pre-0024 rows that carry no timestamp at all.
--
--   WHAT THIS DELIBERATELY DOES NOT DO: refuse anything. No file is rejected
--   for being the second of its day; the identical file is still refused by
--   csv_imports_sha_completed, and that is a different fact.
--
-- NOT A SCHEMA CHANGE. No column, index, constraint or grant moves. This
-- re-issues commit_csv_import from 0042 with one clause added to each of the
-- three statements that reconcile against an earlier file -- kept_by_hand,
-- reverted, removed. Diff it against 0042 and those three hunks are the whole
-- of the difference.
