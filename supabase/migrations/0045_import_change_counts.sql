-- 0045 · a commit says what it CHANGED, not only what it wrote
--
-- WHAT WAS MISSING, in the requester's words
--   "Improve the feedback when a user uploads the same CSV again without any
--   changes ... if the CSV was already imported and all attendance is already
...
--
--   commit_csv_import has always answered `present_or_extra` -- how many rows
--   the file NAMED. That number is identical for a file importing twelve
--   women for the first time and for the same file re-exported an hour later
--   changing nothing at all: "12 marked present" both times. There was no way
--   for the screen to tell those apart, so it told the operator the same
--   thing about both.
--
-- WHAT THIS ADDS
--   Three counters over the rows this file names, decided per row BEFORE the
--   upsert, from the row that is already on the register:
--
--     added      -- no live attendance row for her at this session yet
--     updated    -- a row exists and this file CHANGES its status (the
--                   ordinary case: absent, because an earlier file or the
--                   absent sweep wrote her, and this file names her)
--     unchanged  -- a row exists and already says what this file says, so
--                   nothing about her moves. A mark somebody made BY HAND
--                   counts here too: the upsert deliberately leaves her
--                   status alone (0037), so the register does not change.
--
--   and one more over the sweep that follows them:
--
--     absent_added -- expected members this file does not name who had no
--                     row at all, now recorded absent.
--
--   STATUS IS WHAT THESE COUNT. A re-import still refreshes the evidence on
--   an unchanged row -- minutes, the spelling the file used, which import
--   wrote last -- and that is not a change to the register, which is the
--   thing the operator is being told about.
--
--   added + updated + absent_added + reverted + removed = 0 is what "there is
--   nothing to update" MEANS, and it is now a fact the caller can read rather
--   than a guess it has to make from `present_or_extra`.
--
-- WHAT THIS DOES NOT CHANGE
--   Not one write. Every insert, update and delete below is 0044's, character
--   for character, and no duplicate attendance record was ever possible:
--   attendance_unique_live (0008) is one live row per (session, member) and
--   every path here goes through it. This migration only counts.
--
-- WHAT IT RE-ISSUES
--   commit_csv_import, from 0044 (meeting-instance override scoping), which
--   re-issued it from 0042. Diff against 0044 and the whole of it is four
--   declarations, one lookup before the upsert, the classification that
--   follows it, `returning 1` on the sweep, and the `changes` key.
--
--   ORDER MATTERS AND IS WHY THIS IS 0045. `create or replace` means the last
--   migration to name this function wins outright: numbered 0044 alongside
--   the instance-scoping one, whichever sorted later would have silently
--   dropped the other's work. The three `meeting_started_at` clauses below
--   are 0044's, carried forward unchanged.
